...
dontkillme(void)
{
	...
	if (!(f = fopen(oomfile, "w"))) {
		if (errno == ENOENT)
			return;
		die("slock2: fopen %s: %s\n", oomfile, strerror(errno));
	}
	fprintf(f, "%d", OOM_SCORE_ADJ_MIN);
	if (fclose(f)) {
		if (errno == EACCES)
			die("slock2: unable to disable OOM killer. "
			    "Make sure to suid or sgid slock2.\n");
		else
			die("slock2: fclose %s: %s\n", oomfile, strerror(errno));
	}
}
#endif
...
gethash(void)
{
	...
	if (!(pw = getpwuid(getuid()))) {
		if (errno)
			die("slock2: getpwuid: %s\n", strerror(errno));
		else
			die("slock2: cannot retrieve password entry\n");
	}
	hash = pw->pw_passwd;
	...
	if (!strcmp(hash, "x")) {
		...
		if (!(sp = getspnam(pw->pw_name)))
			die("slock2: getspnam: cannot retrieve shadow entry. "
			    "Make sure to suid or sgid slock2.\n");
		hash = sp->sp_pwdp;
	}
#else
	if (!strcmp(hash, "*")) {
		...
		if (!(pw = getpwuid_shadow(getuid())))
			die("slock2: getpwnam_shadow: cannot retrieve shadow entry. "
			    "Make sure to suid or sgid slock2.\n");
		hash = pw->pw_passwd;
#else
		die("slock2: getpwuid: cannot retrieve shadow entry. "
		    "Make sure to suid or sgid slock2.\n");
#endif /* __OpenBSD__ */
	}
#endif /* HAVE_SHADOW_H */
...
lockscreen(Display *dpy, struct xrandr *rr, int screen)
{
	...
	if (ptgrab != GrabSuccess)
		fprintf(stderr, "slock2: unable to grab mouse pointer for screen %d\n",
		        screen);
	if (kbgrab != GrabSuccess)
		fprintf(stderr, "slock2: unable to grab keyboard for screen %d\n",
		        screen);
	return NULL;
}
...
main(int argc, char **argv) {
	...
	if (!(pwd = getpwnam(user)))
		die("slock2: getpwnam %s: %s\n", user,
		    errno ? strerror(errno) : "user entry not found");
	duid = pwd->pw_uid;
	errno = 0;
	if (!(grp = getgrnam(group)))
		die("slock2: getgrnam %s: %s\n", group,
	...
	if (!(dpy = XOpenDisplay(NULL)))
		die("slock2: cannot open display\n");

	/* drop privileges */
	if (setgroups(0, NULL) < 0)
		die("slock2: setgroups: %s\n", strerror(errno));
	if (setgid(dgid) < 0)
		die("slock2: setgid: %s\n", strerror(errno));
	if (setuid(duid) < 0)
		die("slock2: setuid: %s\n", strerror(errno));
	...
	if (argc > 0) {
		switch (fork()) {
		case -1:
			die("slock2: fork failed: %s\n", strerror(errno));
		case 0:
			if (close(ConnectionNumber(dpy)) < 0)
				die("slock2: close: %s\n", strerror(errno));
			execvp(argv[0], argv);
			fprintf(stderr, "slock2: execvp %s: %s\n", argv[0], strerror(errno));
			_exit(1);
		}
	}
